...
console.log(`Found ${rawLines.length} lines in the database`);
...
console.log(`Processed ${uniqueTrains.length} unique trains`);
...
// Total trains in database: ${rawLines.length}
// Unique trains processed: ${trainRoutes.length} trains
...
export function searchTrains(query: string): TrainRoute[] {
  ...
  ).slice(0, 50); // Return more results for better search
}
`;
...
console.log(`✓ First 5 trains:`, uniqueTrains.slice(0, 5).map(t => `${t.trainNumber} - ${t.trainName}`));
console.log(`✓ Last 5 trains:`, uniqueTrains.slice(-5).map(t => `${t.trainNumber} - ${t.trainName}`));